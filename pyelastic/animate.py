...
class Animation:
    """
    """
    def __init__(self, alpha=None, beta=None, seed=None, cores=None, fps=30,
                 pendulums=1):
        self.dpi = 100
        self.size = 712
    ...
    def main(self, i):
        """
        """
        fig = plt.figure(figsize=(self.size / self.dpi, self.size / self.dpi),
                         dpi=self.dpi)
        for pendulum in self.pendulums:
            self.plot_frame(pendulum, i)

        fig.set_size_inches(self.size / self.dpi, self.size/ self.dpi,
                            forward=True)
    ...
    def animate(self, save_movie=True):
        ...
        for b in self.beta:
            ds = ElasticPendulum(dt=1. / self.fps, t_end=15., alpha_0=self.alpha, beta_0=b)
            _ = ds.integrate()
